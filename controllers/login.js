      ...
      .then(async (user) => {
        if (user.rows.length > 0) {
          ...
          if (decryptedPassword) {
            ...
            res.cookie('jwt', jwtObject.token, { httpOnly: true, maxAge: jwtObject.expriry * 1000, sameSite: 'none', secure: 'secure'});
            res.cookie('is-logged-in', true, { maxAge: jwtObject.expriry * 1000, sameSite: 'none', secure: 'secure'})
            res.status(200).send({
